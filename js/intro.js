...
window.addEventListener('load', function () {
  ...
  var VS = [
    ...
    '  vec2 c = vec2(aPos.x / (2.0 * asp), aPos.y / 1.15);',
  ...
  function scatterFromEdges(i) {
    /* Scatter randomly across the full screen — no rectangular edge bias.
       World bounds match the new shader: X in [-2.0*asp, 2.0*asp], Y in [-1.15,1.15]
       We spread wider (1.6x) so particles truly come from off-screen all over. */
    var asp = W / H;
    var rx  = 2.0 * asp * 1.6;   /* wider than screen */
    var ry  = 1.15 * 1.6;
